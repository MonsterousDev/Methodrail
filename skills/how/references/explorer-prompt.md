...
2. **Trace the flow.** Follow the call chain from the entry point. Read each function. Understand data flow, control-flow branches, and how values transform.
3. **Trace state.** Where is authoritative state stored? What mutates it? What reads it? What is ephemeral versus durable?
4. **Map the key abstractions.** What types, interfaces, services, or classes are central? Read their definitions. Understand what they represent and why they exist.
5. **Find the boundaries.** Where does this subsystem interface with others? What goes in, what comes out?
6. **Look for the non-obvious.** Anything surprising? Anything that looks like a historical artifact? Anything a newcomer would misunderstand?
...
### Flow
The execution flow step by step. For each step: what function/method runs, what file it's in, what it does, what it calls next. Include the data that flows between steps, important control-flow branches, and where state is read or written.
