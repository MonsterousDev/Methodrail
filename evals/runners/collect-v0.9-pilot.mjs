...
const pilot = process.env.METHODRAIL_PILOT_DIR ?? "/tmp/methodrail-pilot-20260901-v09";
const freeze = existsSync(join(pilot, "METHODRAIL_HEAD"))
  ? readFileSync(join(pilot, "METHODRAIL_HEAD"), "utf8").trim()
  : "unknown";

const fixtures = ["decision-ladder", "knowledge-reconciliation-v0.9", "architecture-deepening"];
const runs = [];
...
for (const spec of runs) {
  ...
  const example = {
    ...
    failure_modes: [],
    notes: `Live v0.9 project-artifact-interoperability pilot 2026-09-01. Freeze HEAD ${freeze} (uncommitted v0.9 graders). Worktree ${dest}. Cursor runs used isolated Task subagents on /tmp copies. Codex used codex exec --ignore-user-config --skip-git-repo-check --sandbox workspace-write --ignore-rules -m gpt-5.5. Extra copy; canonical example JSON was not replaced.`,
    provenance: "live",
    capture,
    artifacts: {
