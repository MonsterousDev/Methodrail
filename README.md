...
v0.7.0 is Harness Intelligence. Methodrail can promote evidence-backed project knowledge, index it for future sessions, validate its provenance, check freshness against relevant repository changes, and selectively reuse or reconcile it. It is not an autonomous memory service. Constructed reuse and refresh specs pass. A 6-pair live pilot was outcome-neutral on reuse (capable baselines copied the existing dispatchOnce path) and helped on refresh when Methodrail proposed a note update without rewriting the note. That is a low-n pilot, not a general performance claim. Simple work is still required to stay cheap. Cursor is first-class; reusable skills remain compatible with Agent Skills-oriented Claude Code and Codex setups.
